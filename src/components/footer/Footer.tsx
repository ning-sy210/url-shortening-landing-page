...
const socialLinks = [
  {
    icon: (
      <svg
        ...
        width="24"
        height="24"
        className="fill-white transition-colors duration-200 hover:fill-primary-1"
      >
...
const Footer = () => {
  return (
    <footer className="full-width child-grid pt-[3.375rem] pb-14 bg-neutral-4 text-white desktop:pt-[4.5rem] desktop:pb-[4.125rem] desktop:text-start">
      <div className="content grid justify-items-center desktop:flex desktop:items-start">
        ...
        {/* footer links */}
        <ul className="mt-11 grid gap-y-8 desktop:mt-0 desktop:ml-auto desktop:grid-cols-3 desktop:gap-x-20">
          {navSections.map((section) => (
            <li key={section.label}>
              <NavSection label={section.label} links={section.links} />
            </li>
          ))}
        </ul>

        {/* social icons */}
        <ul className="flex items-center gap-x-6 mt-11 desktop:mt-0 desktop:ml-[4.25rem]">
...
const NavSection = ({ label, links }: NavSectionType) => {
  return (
    <section className="content">
      <h4 className="text-mobile-h4 mb-[0.875rem]">{label}</h4>
      <ul className="grid gap-y-[0.1875rem] text-gray-300">
        {links.map((link) => (
          <li key={link.label}>
            <a
              href={link.path}
              className="text-mobile-p-sm font-normal transition-colors duration-200 hover:text-primary-1"
            >
              {link.label}
            </a>
          </li>
